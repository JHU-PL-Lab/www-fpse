...
* **Important note**: if you use a search engine to look up e.g. "OCaml Set" to see how the OCaml Set module is defined you will likely not get the `Core` version and it can be very confusing as it is similar.  Even if you search "OCaml Set Core" you will likely get an outdated version of `Core.Set`.  So, please **bookmark the above link, and avoid countless hours of fruitless debugging because you are using the wrong library docs**.
