...
* There are corresponding `.mli` files that state which functions you must have in your submission. You will not submit these files, so do not edit them. If you edit them, then Gradescope may fail to run your code.  Note that for this assignment we also list all functions you need to define in the `.ml` files, so you can largely ignore them now; it is more of a warm-up to how real OCaml development works.
...
* Consult the [OCaml Intro lecture notes](https://pl.cs.jhu.edu/fpse/lecture/ocaml-intro.html), and if you want to re-watch any lecture they are on Panopto as per the link pinned on Courselore.
* [Real World OCaml Chapter 1](https://dev.realworldocaml.org/guided-tour.html) is another tutorial introduction in a somewhat different order than we are doing. They use the `Core` standard library overlay, so their library functions may look different than yours.
...
* You are encouraged to work with other people on the assignment. You just need to list the names of people you worked with. However, remember that you should submit your own write up of the answers. **Copying of solutions is not allowed**. For the full collaboration policy see [here](https://pl.cs.jhu.edu/fpse/integrity.html).
...
* Please submit your draft assignment at least once well ahead of the deadline, so you do not find some problem right at the deadline.
