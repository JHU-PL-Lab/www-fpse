...
* These are languages designed with FP style in mind from the start: immutable variables and data structures, full higher-order functions, currying, etc.
  ...
  - The term "persistent" has a longstanding meaning of surviving over multiple runs of a program so I view "persistent data structure" as a misleading term.
* There are generally two "schools"
    - ML school: static types, type inference, polymorphism, pattern matching (OCaml, Standard ML, ReScript, Haskell, F#, etc)
    - Lisp school: dynamically typed (Lisp, Scheme, Clojure, Elm, etc)
...
* [ReScript](https://rescript-lang.org) is an interesting beast, it is a fork of OCaml in terms of features
  ...
  - You can convert `.res` to `.bs.js` to run in a browser
  - [Here](https://rescript-lang.org/try) is a playground where you can see how `.res` is turned into `.js.bs`.

* ReScript is a fork of [Reason](https://reasonml.github.io) but that project seems to be languishing now
  - Reason is "full OCaml" (every single feature) but with a more modern syntax
  - [This demo](https://reasonml.github.io/en/try) shows the close relation of Reason, OCaml, and JavaScript
...
* [Here](https://github.com/jihchi/rescript-react-realworld-example-app) is an example of a full browser app written in ReScriptReact.
  - ReScriptReact counts as "OCaml" for the course projects, an option if you already know React.
...
* Lisp was the very first functional programming language, from the late 50's
  ...
  - Allows mutation everywhere (no immutable `let`), but "only mutate when really needed".
...
* [Clojure](https://clojure.org) is another more modern Lisp dialect
  - Has more immutablility built-in and can more cleanly support parallelism 
...
* There is not necessarily good library support or integration
  ...
  - To "really do" FP you need immutable data structures and variables
...
* Python will also eventually have pattern matching
...
* It is the key thing missing from C: C has *function pointers* you can pass around, but no closures.
*   OCaml example:
...
* Note how `x` is a function parameter and is remembered in spite of function returning, means a reference to `x` is needed in the closure
