...
module Make_pair_smartest(Datum1 : DATUM)(Datum2 : DATUM)
...
(*
  We turn a module into a first class value with the `(module M)` syntax.
  Often the type checker needs some help, so we annotate it with a type like
  `(module M : MODULE_TYPE)`.
*)

let mcell = (module Example_pair_smartest : EPS)
...
(* When to use first class modules? Nowadays, they can be used to avoid single-
  purpose functors--functors that derive a module with just one function. *)
...
(* To get a list_fold_leftM, we have to call the functor, name the resulting
  module, and then access from that module. *)
...
(* With module-dependent functions, we just pass the module on the fly! *)
let sum_if_pos' ls =
